...
const ScooterIdentity = ({ 
  ...
  const NicknameDisplay = () => {
    if (!editable) {
      return nickname ? (
        <p className="text-base text-mineral/70 italic font-light">
          « {nickname} »
        </p>
      ) : null;
    }
        ...
        <AnimatePresence mode="wait">
          {isEditing ? (
            ...
            >
              <input
                ...
                className="bg-transparent border-b border-mineral/30 focus:border-mineral 
                          outline-none text-base text-carbon/70 italic font-light 
            ...
            <motion.button
              ...
              className="group flex items-center gap-1.5 text-base text-mineral/70 italic font-light 
  ...
  if (variant === 'desktop') {
    return (
      <div className={cn("flex flex-col items-center gap-1 ml-[180px]", className)}>
        <span className="px-2.5 py-0.5 bg-mineral/10 backdrop-blur-sm rounded-full text-[9px] font-semibold 
                        text-mineral uppercase tracking-[0.15em] border-[0.5px] border-mineral/20">
          {brandName}
        </span>
        <div className="font-display text-xl text-carbon bg-white/80 backdrop-blur-sm 
                      px-5 py-2 rounded-full border-[0.5px] border-mineral/20 flex items-center gap-2">
          <span className="font-bold bg-gradient-to-r from-carbon via-mineral to-carbon bg-clip-text text-transparent">
            {modelName}
          </span>
          {editable ? (
            <NicknameDisplay />
  ...
  // Mobile: Vertical centered layout — Luxury Sport
  return (
    <div className={cn("text-center space-y-2", className)}>
      {/* BRAND — Micro-caps tag */}
      <div className="inline-flex items-center justify-center">
        <span className="px-3.5 py-1 bg-gradient-to-r from-mineral/10 to-mineral/5 backdrop-blur-sm rounded-full 
      ...
      {/* MODEL — Bold Sport/Gaming gradient text */}
      <h1 className="font-display text-4xl md:text-5xl font-bold tracking-tight
                     bg-gradient-to-r from-carbon via-mineral to-carbon bg-clip-text text-transparent
                     leading-tight">
        {modelName}
      </h1>
      
      {/* NICKNAME — Larger, elegant */}
      <NicknameDisplay />
