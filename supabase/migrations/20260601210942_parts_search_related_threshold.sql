-- =====================================================================
-- Seuil "related" plus strict pour search_parts_fuzzy
--   CREATE OR REPLACE de la fonction uniquement (trigger/helper/colonne intacts).
--   WHERE : nom conservé à 0.3 (tolérance fautes), document + compat montés à 0.4.
--   score_nom (GREATEST word_similarity / ILIKE) et match_type INCHANGÉS.
--   Pas de SET pg_trgm (interdit Lovable Cloud) : word_similarity explicite.
-- =====================================================================
...
BEGIN
  ...
  scored AS (
    ...
    WHERE p.published = true
      ...
      AND (
        v_q = ''
        OR word_similarity(v_q, public.f_unaccent(lower(p.name))) >= 0.3
        OR p.search_document ILIKE '%'||v_q||'%'
        OR word_similarity(v_q, p.search_document) >= 0.4
        OR word_similarity(v_q, cp.compat_doc) >= 0.4
      )
  ),
  ranked AS (
    SELECT s.*,
  ...
  FROM ranked r
  ORDER BY (r.match_type_val = 'exact') DESC,
           r.rank_val DESC,
